...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{alignment_img/ATLAS_terminal.png}
	\caption{ATLAS terminal. Tx/Rx apperture is on the right, beacon camera is on the left} 
	\label{fig:atlas_terminal}
\end{figure}

(TODO) PeakSat will use ATLAS-1 terminal, that is an optical terminal designed by the Lithuanian start-up company Astrolight, capable of performing Free Space Optical communication from Low Earth Orbit. ATLAS terminal promises to deliver an 100 Mbps download data rate at a wavelength of $1550\pm 3 nm$, and an 1Mbps uplink data rate at a wavelength of $1535 \pm 5 nm$. The Atlas down-link laser beam will be referred to as the transmit beam, Tx, and the up-link laser beam will be referred to as receive beam, Rx. In this work we are not going to mention the Rx Beam, as it used the same aperture as Tx, and from our standpoint their operations are identical. \textit{We should not confuse the Rx beam, that is used for data uplink to the satellite, and the Beacon Camera (BC) that is used for Tx,Rx pointing. In this work, we will mention the Beacon Camera several times, whereas the Rx beam not at all.}  

The optical system of the Tx consists of: 1) Laser Diode and a Single Mode Optical Fiber 2)A Mode Fiber Collimator, used to parallerize the light rays 3) Fast Steering Mirror (FSM) used for precise pointing of the beam, and and 4) optical telescope used for further collimating the free space light beam. The telescope has an aperture size of 32.4 nm. (\cref{fig:atlas_terminal}, the bigger apperture on the right). 
